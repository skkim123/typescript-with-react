...
function getAvailableProducts(
  ...
  // 여기에 구현
  return products.filter((v) => v[2] === true);
}
...
type ProductWithCategory = { name: string; price: number; category: string };
...
function getProductsByCategory(category: string): string[] {
  // 여기에 구현
  // 배열 함수에는 타입스크립트 타입 추론이 똑똑하게 안 들어가서, 찾아보니 타입 가드라는 것이 있어서 사용했습니다.
  return _products
    .filter(
      (product): product is ProductWithCategory => product.category === category
    )
